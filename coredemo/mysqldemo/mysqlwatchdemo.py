...
def main():
    try:
        conn = MySQLdb.connect(host=dbconn.DB_HOST, port=int(dbconn.DB_PORT), user=dbconn.DB_USER,
                               passwd=dbconn.DB_PASS, charset='utf8')
    ...
    while True:
        try:
            ...
            print 'qps_s = %s , qps_ques = %s , tps_iud = %s ,tps_Com_rol = %s' % (
            qps_s, qps_ques, tps_iud, tps_Com_rol)
